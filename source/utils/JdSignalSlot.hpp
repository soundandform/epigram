...
//  JdSignalSlot.hpp
//
//  Created by Steven Massey on 7/20/15.

 "stolen" from: https://testbit.eu/~timj/blogstuff/simplesignal.cc I just wanted to changed the semantics:
 
 jd::signal <> x_signal;  // void (void) signal
 
 auto connection = x_signal << jd::slot (obj, & Class::Function);
 
 x_signal ();  // OR,
 x_signal.emit ();
 
 x_signal.disconnect (connection);
 


*/

#ifndef __JdSignalSlot__
#define __JdSignalSlot__
...
namespace jd
{
	...
	namespace Lib
	{
		...
		class ProtoSignal<R (Args...), Collector> : private CollectorInvocation<Collector, R (Args...)>
		{
			...
			SignalLink   *callback_ring_ = nullptr; // linked ring of callback nodes
			...
			ProtoSignal (const ProtoSignal& i_other)
			{
				ensure_ring ();
				SignalLink * signal = i_other.callback_ring_;
				...
				if (signal)
				{
					...
					{
						if (signal->function)
							callback_ring_->add_before (signal->function);
						
						signal = signal->next;
					}
					while (signal != i_other.callback_ring_);
				}
			}
			...
			connection_t connect (const CbFunction &cb)
			{
			...
			void clear ()	// doesn't clear default slot
			{
				if (callback_ring_)
				{
					while (true)
					{
						SignalLink * link = callback_ring_->next;
						
						if (link == callback_ring_)
							return;
						
						callback_ring_->remove_sibling (size_t(link));
					}
				}
			}
			
			connection_t operator << (const CbFunction &cb)
			{
				return connect (cb);
			}
			...
			CollectorResult operator () (Args... i_args)
			{
				return emit (i_args...);
			}
	...
	template <typename T>
	struct Hack { typedef T type; };

	typedef Hack <void ()>::type voidFunc_t;

	template <typename SignalSignature = voidFunc_t, typename Collector = Lib::CollectorDefault <typename std::function <SignalSignature>::result_type>>
	struct signal :
...
#endif /* defined(__JdSignalSlot__) */
